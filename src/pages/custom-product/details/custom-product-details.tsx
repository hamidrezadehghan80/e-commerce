...
import { Minus, Plus, ShoppingBag, Star, Trash } from "@phosphor-icons/react";
...
export default function CustomProductDetails() {
  ...
  const { state: customProducts } = customProductContext;

  const product = customProducts.products.find((product) => product.id === +id);
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-4">
                ...
                <Button
                  ...
                  size={"sm"}
                  onClick={() =>
                    navigate(AppRoutes.EditCustomProduct + "/" + id)
                  }
                >
                  Edit
                </Button>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-end gap-2">
                  <div className="flex items-center gap-0.5">
                    {[
                      ...Array.from({ length: starCount }, (_, index) => index),
                    ].map((index) => (
                      <Star
                        key={index}
                        weight="fill"
